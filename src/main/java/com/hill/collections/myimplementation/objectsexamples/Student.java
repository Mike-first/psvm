package com.hill.collections.myimplementation.objectsexamples;
...
public class Student implements Serializable {
    ...
    */
    ...
    public static ArrayList<Student> generateStudent(int quantity) {
        if (quantity <= 0 | quantity > 100) throw new IllegalArgumentException("quantity should be 1 to 100");
        ArrayList<Student> st = new ArrayList<>();
        for (int i = 0; i < quantity; i++) {
            int age = (int) (16 + Math.random() * 40);
            st.add(new Student(randomString(), age));
        }
        return st;
    }
    ...
    public static Comparator<Student> ByAge = (s1, s2) -> {
        if (s1 != null && s2 != null) {
