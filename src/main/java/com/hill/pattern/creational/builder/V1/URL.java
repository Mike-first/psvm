//this is one of the variations of the builder pattern (Client - Builder - Product)
//the constructor is hidden, the object (in our case - URL, see Wiki) can only be obtained by methods of the built-in Builder class (in our case it is called Composer)
//Composer has fields that duplicate all the URL fields and methods for setting these fields, returning the Composer object, as well as the compose() method, returning the URL
//the compose() method implements logic that determines the set of necessary fields without which the object cannot be created,
//the methods returning Composer also implement some logic (for example, argument validation). The identical output of these
//methods allows you to write them through a dot in a chain. or access the Composer object at any time between its creation and calling the compose() method.
//Composer is a static inner class, which allows us to create its object without creating an outer class

//Builder (this variation of it) can be used to build an immutable object (reflection does not count), all properties of which are defined at the creation stage
//for this we hid the constructor, defined all private fields and did not create setters.

//in this example, getters for private URL fields are not described and such code is not applicable to a real task, but for studying the pattern, the toString() method is enough

package com.hill.pattern.creational.builder.V1;
